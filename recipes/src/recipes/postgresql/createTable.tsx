...
export const CreateTable: React.FC<IRecipeProps> = ({
  ...
  for (var i = 0; i <= columnsArr.length - 1; i++) {
    ...
    if (i === columnsArr.length - 1) {
      queryDataTypes = queryDataTypes.concat("\t\t\t\t\t\t{col", ii as string, "} ", dataTypesArr[i])
      queryColumns = queryColumns.concat("\t\t\t\t\tcol", ii as string, "=sql.Identifier(\"", columnsArr[i], "\")")
      // queryDataTypes += "\t\t\t\t\t\t" + "{a" + (i+1) + "}" + " " + dataTypesArr[i]
      // queryColumns += "\t\t\t\t\ta" + (i+1) + '=sql.Identifier("' + columnsArr[i] + '")'
      break
    }
    queryDataTypes = queryDataTypes.concat("\t\t\t\t\t\t{col", ii as string, "} ", dataTypesArr[i], ",\n")
    queryColumns = queryColumns.concat("\t\t\t\t\tcol", ii as string, "=sql.Identifier(\"", columnsArr[i], "\"),\n")
