...
export const BackupTable: React.FC<IRecipeProps> = ({
  ...
  const [oldTable, setOldTable] = useState("old_table");
  const [newTable, setNewTable] = useState("table_backup");
  ...
  useEffect(() => {
    ...
    src += `        try:\n`;
    src += `            cur.execute(\n`;
    src += `							sql.SQL("CREATE TABLE {new} AS TABLE {old}").format(\n`;
    src += `							  old=sql.Identifier("${oldTable}"),\n`;
    src += `							  new=sql.Identifier("${newTable}"),\n`;
    src += `						))\n`;
  ...
  }, [conn, newTable, oldTable]);

  useEffect(() => {
    if (metadata) {
      if ("mljar" in metadata) metadata = metadata.mljar;
      if (metadata["conn"] !== undefined) setConnection(metadata["conn"]);
      if (metadata["oldTable"] !== undefined) setOldTable(metadata["oldTable"]);
      if (metadata["newTable"] !== undefined) setNewTable(metadata["newTable"]);
    }
  }, [metadata]);
