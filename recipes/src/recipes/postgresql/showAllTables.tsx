...
export const ShowAllTables: React.FC<IRecipeProps> = ({
  ...
  if (variablesStatus === "loaded" && !connections.length) {
    return (
      <div className="bg-white dark:poc-bg-slate-800 p-4 rounded-md">
        <p className="text-base text-gray-800 dark:text-white">
          There are no connection objects in your notebook. You can open a new
          connection to run the query.
        </p>
      </div>
    );
  }
...
export const ShowAllTablesRecipe: IRecipe = {
  ...
  // len: 176
  description:
    "List all table on your connection. Use advanced options to show table schema. To show tables from other databases change your connection. Credentials are loaded from .env file.",
  shortDescription:
    "List all table on your connection. Use advanced options to show table schema. To show tables from other databases change your connection. Credentials are loaded from .env file.",
  codeExplanation: `
