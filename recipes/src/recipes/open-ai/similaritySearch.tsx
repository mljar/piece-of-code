...
export const SimilaritySearch: React.FC<IRecipeProps> = ({
  ...
  useEffect(() => {
    if (metadata) {
        ...
        if (metadata["embeddings"] !== undefined) setEmbeddings(metadata["embeddings"]);
...
export const SimilaritySearchRecipe: IRecipe = {
  ...
  longName: "Search the cosine similarity using OpenAI API and Python",
  parentName: "OpenAI",
  description: "Learn how to find text most similar to a user query using OpenAI's API in Python. This guide covers creating embeddings for the user query, calculating cosine similarity with precomputed embeddings, and retrieving the most similar text chunk. Enhance your text analysis and search capabilities by integrating these steps into your Python applications.",
  shortDescription: "Learn how to find text most similar to a user query using OpenAI's API in Python. This guide covers creating embeddings, calculating cosine similarity, and retrieving the most similar text chunk.",
  ...
  requiredPackages: [
    ...
    { importName: "sklearn", installationName: "scikit-learn", version: ">=1.5.1" }
  ],
  docsUrl: DOCS_URL,
  defaultVariables: [
    {
